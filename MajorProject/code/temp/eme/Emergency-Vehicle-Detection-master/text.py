...
# decode the predictions and apply non-maxima suppression to suppress weak, overlapping bounding boxes
...
for (startX, startY, endX, endY) in boxes:
    ...
    # in order to obtain a better OCR of the text, apply a bit of padding surrounding the bounding box
    ...
    # in order to apply Tesseract v4 to OCR text, we must supply (1) a language, (2) an OEM flag of 4, indicating
    # that the we wish to use the LSTM neural net model for OCR, and finally (3) an OEM value, in this case, 7,
    # which implies that we are treating the ROI as a single line of text
...
for ((startX, startY, endX, endY), text) in results:
    ...
    # strip out non-ASCII text so we can draw the text on the image using OpenCV, then draw the text and a bounding box
    # surrounding the text region of the input image
